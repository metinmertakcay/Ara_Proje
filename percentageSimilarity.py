...
def percent(answers, cevaplar):
    i = 0  # answerda indis olarak kullandım
    sayac = 0
    while (i < len(answers)):
        if(i % 2 == 1):
            index = ((soruNo - 1) * 2) + 1
            if(answers[i] == cevaplar[index]):
                sayac = sayac + 1
        else:
            soruNo = answers[i]
        i = i + 1 # bir sonraki soruya geçiliyor
    return sayac
...
try:
    file = codecs.open("değinilmiştir.txt","r","utf-8-sig")
...
print("Sözcükler Bazında Yapılan Karşılaştırmada Doğru Oranı : %", percent(answers,cevaplarListesi)*100/(len(answers)/2))
...
print("6 Harflik Sözcükler Bazında Yapılan Karşılaştırmada Doğru Oranı : %", percent(answers2,cevaplarListesi)*100/(len(answers2)/2))
...
print("3-Gram Bazında Yapılan Karşılaştırmada Doğru Oranı : % ", percent(answers3,cevaplarListesi)*100/(len(answers3)/2))
